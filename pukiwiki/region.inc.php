...
EOD;
	...
	function buildBracketHtml(){
		...
		return <<<EOD
<td id=rgn_bracket$this->callcount style="font-size:1pt;border:gray 0px;$bracketstyle">&nbsp;</td>
EOD;
	}

	// ■ 縮小表示しているときの表示内容。
	function buildSummaryHtml(){
		$summarystyle = ($this->isopened) ? "display:none;" : "display:block; white-space: nowrap;";
		return <<<EOD
<td id=rgn_summary$this->callcount style="color:black;$summarystyle">$this->description</td>
EOD;
	}

	// ■ 展開表示しているときの表示内容ヘッダ部分。ここの<td>の閉じタグは endregion 側にある。
	function buildContentHtml(){
		$contentstyle = ($this->isopened) ? "display:block;white-space: nowrap;" : "display:none;white-space: nowrap;";
		return <<<EOD

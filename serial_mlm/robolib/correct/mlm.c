...
//#define _MLM_DEBUG
#undef _MLM_DEBUG
#define _MLM_DEBUG_CORRECT
//#undef _MLM_DEBUG_CORRECT
...
#define MAX_CORRECT_DTHETA ((int64_t)7585) // theta(pi/4)
...
static position_t calc_difference(void)
{
#ifndef __INSTALL_OMNI_SET__
    position_t diff = {0, 0, 0, 0, 0}; // マップの誤差
#else
    position_t diff = {0, 0, 0, 0, 0, 0}; // マップの誤差
#endif
    int32_t avg_x, avg_y, avg_xd, avg_yd; // x, y, xd, ydの平均
    int32_t dt_denom, dt_numer; // 角度誤差の分母・分子
...
#ifdef _MLM_DEBUG
        log_message(stream_mlm, "# denom is 0 #\n");
#endif
        return diff;
    }else{

...
{
    if (argc != 5)
        abort_("Usage: program_name <file_in> <file_out> offsetX offsetY");
    int offsetX = -atoi(argv[3]);
    int offsetY = -atoi(argv[4]);

    auto image = getImage(string(argv[1]));
    rep(y, image.size()) rep(x, image[y].size()) {
        int r, g, b; tie(r, g, b) = image[y][x];
        if ((y + offsetY) % 13 == 0 || (x + offsetX) % 6 == 0) 
            r = 0, g = b = 255;
        image[y][x] = tie(r, g, b);
    }

    setImage(string(argv[2]), image);

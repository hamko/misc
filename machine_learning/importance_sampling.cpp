...
double sample_uniform(void) {
    return (double)(randxor() % UINT32_MAX) / UINT32_MAX;

}
double sample_standard_gauss(void) {
    return sqrt(-2 * log(sample_uniform())) * cos(2*M_PI*sample_uniform());
}
double sample_gauss(double mu, double sigma) {
    return sample_standard_gauss() * sigma + mu;
}

double pdf_gauss(double x, double mu, double sigma) {
    return exp(-(x-mu)*(x-mu)/2.0/sigma/sigma)/sqrt(2*M_PI*sigma*sigma);
}
...
int main(void) {
    ...
    {
        // E_f[I(x>=5)] = sum_f(I(x>=5) * f(x))
        double ret = 0;
        rep(_, num) {
            double x = sample_standard_gauss();
            ret += (x >= 5) * pdf_gauss(x, 0, 1);
        }
        cout << ret / num << endl;
    }

    {
        // E_f[I(x>=5)] = sum_g(I(x>=5) * f(x) / g(x))
        // g = N(5, 1)
        double ret = 0;
        rep(_, num) {
            double x = sample_gauss(5, 1);
            ret += (x >= 5) * pdf_gauss(x, 0, 1) / pdf_gauss(x, 5, 1);
        }
        cout << ret / num << endl;
    }

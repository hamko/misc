...
#ifdef DEBUG_MODE
void print_particles(pfsys_t* pfsys)
{
    int i, j;

    for (i = 0; i < pfsys->pnum; i++) {
        for (j = 0; j < pfsys->dnum; j++) 
            printf("%f ", PFELEM(pfsys, i, j));
        printf("%f\n", pfsys->weight[i]);
    }
}
#endif
...
static void resample_particle(pfsys_t* pfsys)
{
    ...
    for (i = 0; i < pfsys->pnum; i++) {
        ...
        memcpy(&(PFELEM(pfsys, i, 0)), &(tmpparticle[PFINDEX(pfsys, n, 0)]), sizeof(float) * pfsys->dnum);
...
static void calc_particle_weight(pfsys_t* pfsys, float* mdat)
{
    ...
    float wsum = 0.0f;
    for (i = 0; i < pfsys->pnum; i++) { 
        pfsys->weight[i] = pfsys->likelyhood_func(&PFELEM(pfsys, i, 0), mdat);
        wsum += pfsys->weight[i];
    }
    if (wsum == 0) 
        printf("#! wsum = 0!\n");
...
void get_particle_mean_pfsys(float* mean_particle, pfsys_t* pfsys)
{
    ...
    for (i = 0; i < pfsys->pnum; i++)
        for (j = 0; j < pfsys->dnum; j++)
            mean_particle[j] += PFELEM(pfsys, i, j);
    for (j = 0; j < pfsys->dnum; j++)
...
void get_particle_unipolar_pfsys(float* unipolar_particle, pfsys_t* pfsys)
{
    ...
    for (i = 0; i < pfsys->dnum; i++)
        unipolar_particle[i] = 0;
    for (i = 0; i < pfsys->pnum; i++) 
        for (j = 0; j < pfsys->dnum; j++) 
            unipolar_particle[j] += PFELEM(pfsys, i, j) * pfsys->weight[i];
}
